...
def run_ML_pipeline(report, file_path, model_id):
    ...
    """
    # Load and preprocess the user-provided data
    ...
    # Ensure the columns of train_X match those of test_x
    ...
    if model_id == 'NB':
        ...
        clf = RandomForestClassifier(n_estimators=1000, criterion='gini', max_depth=None, min_samples_split=2, random_state=seed, n_jobs=-1)
    elif model_id == 'SVM':
    ...
    if report == 'confusion_matrix':
        conf_mat_plot = ConfusionMatrixDisplay.from_estimator(clf, test_x, pred_y).ax_
        return conf_mat_plot
    elif report == 'roc_auc_curve':
        roc_auc_curve_plot = RocCurveDisplay.from_estimator(clf, test_x, pred_y).ax_
        ...
        if model_id == 'RF':
            ...
            raise NotImplementedError("Feature importance is only available for Random Forest.")
